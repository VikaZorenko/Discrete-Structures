...
bool modulo(int val, int begin, int end) { //функція для перевірки модуля
    ...
    if (sum > begin && sum < end) { // перевіряємо на відповідність умові
        return true; 
    }
    return false;
	};
...
void findHelper(TreeNode *r, int begin, int end) {
    ...
    if (modulo(r->val, begin, end)) { //якщо значення відповідає умові задачі
