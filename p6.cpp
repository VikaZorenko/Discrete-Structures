...
bool isPrime(int val) { // перевіряємо даною функцією чи число просте 
		if (val <= 0) return false; //якщо менше 0 число, воно не просте повертаємо false
		for (int i = 2; i < val/ 2; i++) { //у циклі перевіряємо чи ділиться дане значення на щось крім 1 та самого себе
...
void findHelper(TreeNode *r) {
    ...
    if (isPrime(r->val)) { //якщо значення відповідає умові задачі
